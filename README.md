...
| 运维知识库 | `knowledge` | 沉淀案例（只读） | 案例编号、设备类型、缺陷类型、处理措施、处理耗时 |
...
- 运维知识库不登记、不存表：案例由 `services/knowledge.py` 实时关联缺陷登记与
  已验收消缺单生成（同一缺陷重复登记的消缺单按缺陷编号去重，耗时取首次处理），
  因此缺陷/消缺的既有登记流程不受影响，案例描述与缺陷记录天然保持一致。
