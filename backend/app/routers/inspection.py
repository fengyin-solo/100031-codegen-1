"""巡检任务接口：维护巡检单，覆盖派发巡检、提交巡检结果、作废巡检等动作。"""
...
router = APIRouter(prefix="/api/inspection", tags=["巡检任务"])

service = InspectionService()

LIST_FIELDS = ["巡检单号", "巡检类型", "巡检路线", "巡检人员", "开始时间", "发现问题数", "巡检周期", "巡检状态"]
...
def get_entry(entry_id: int) -> dict:
    ...
    if entry is None:
        raise HTTPException(status_code=404, detail=f"巡检单 {entry_id} 不存在或已归档")
    return entry


@router.post("", response_model=ActionResult)
def create_entry(payload: EntryPayload) -> ActionResult:
    """登记一条巡检单，缺字段时说明原因而不是静默丢弃。"""
    entry, missing = service.create_entry(payload.values)
    if missing:
        return ActionResult(ok=False, message=f"缺少必填字段：{'、'.join(missing)}")
    return ActionResult(ok=True, message="巡检单已登记", entry=entry)


@router.post("/{entry_id}/actions", response_model=ActionResult)
def run_action(entry_id: int, payload: EntryPayload) -> ActionResult:
    """对单条巡检单执行派发巡检、提交巡检结果、作废巡检；不允许的动作会被拦下并说明原因。"""
